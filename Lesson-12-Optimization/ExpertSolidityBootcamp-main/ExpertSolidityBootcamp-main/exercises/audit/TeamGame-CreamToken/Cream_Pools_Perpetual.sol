...
contract PerpetualPool is Ownable {
    ...
    constructor(IERC20 _CREAM, IERC20 _REWARD, uint256 _rewardPerBlock, uint256 _startBlock, uint256 _endBlock, uint256 _entryFee) public {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && stakedCream != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(1e12).div(stakedCream));
        }
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(stakedCream));
        poolInfo.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        CREAM.safeTransferFrom(address(msg.sender), address(this), _amount - fee);
    ...
    function emergencyTransfer(address _to) external onlyOwner returns (bool) {
        if (CREAM == REWARD) {
            return REWARD.transfer(_to, REWARD.balanceOf(address(this)) - stakedCream);
        } else {
     ...
     */
    function setCreamPerBlock(uint256 _rewardPerBLock) external onlyOwner returns (uint256) {
